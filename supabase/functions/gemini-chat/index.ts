...
**핵심 목표가 달성되기 위해 결정적으로 관리되어야 할 영역들을 구조적으로 도출하는 것**입니다.
...
- **S** (Specific): 관리해야 할 대상이 명확함
- **M** (Measurable): 진척 또는 성취 여부를 판단할 수 있음
- **A** (Achievable): 1년 단위로 현실적으로 달성 가능함
- **R** (Relevant): 핵심 목표 달성의 핵심 요인에 해당함
- **T** (Time-bound): 장기 비전이 아닌 집중 가능한 기간 단위임
...
### 응답 형식 (JSON)
{
  "recommendations": [
    {
      "text": "하위 목표 1 (최대 10자)",
      "reason": "이 목표가 핵심 목표 달성에 결정적으로 중요한 이유와 SMART 관점의 설명"
    },
    {
      "text": "하위 목표 2 (최대 10자)",
      "reason": "이 목표가 핵심 목표 달성에 결정적으로 중요한 이유와 SMART 관점의 설명"
...
- 간결한 명사형 어휘로, 괄호 없이 작성하세요
- 액션플랜은 최대 30자 이내로 작성하세요
- Macro한 형태로 작성하세요 (예시: 주 1회 등산, 미라클 모닝, 말보다 행동)
